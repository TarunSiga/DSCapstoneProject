...
if not firebase_admin._apps:
    ...
    cred = credentials.Certificate("localGPT/Demo/login1.json")
...
def main():
    ...
    if not qa_system:
        # st.warning("Initializing QA system...")
        qa_system = retrieval_qa_pipeline(device_type="cpu", use_history=False)
        # st.info("QA system initialized successfully!")

    st.write("How can I help you today?")

    # Input prompt for the user
    col1, col2, col3 = st.columns([8.6, .7, .7])  # Define column layout
    ...
    with col1:
        ...
        if user_input != st.session_state.user_input:
            ...
            st.session_state.chat_history.append({"query": st.session_state.user_input, "response": ""})
            ...
            response = qa_system(st.session_state.user_input)["result"]
            insert_data(collection_name, {"input_text": st.session_state.user_input, "response": response})
            ...
            # Clear the input prompt after submitting the query
            st.session_state.user_input = ""
